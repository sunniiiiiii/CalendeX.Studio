...
1. Scan the whole document and identify every event title (class / activity name) first, regardless of language or layout.
2. Translate ALL extracted information into English. Every text field you return (title, location, notes) MUST be in English. Keep the original-language term in parentheses after the English translation when it aids clarity (e.g. "Microeconomics (微观经济学)").
3. For each identified event, extract its key dates and times from the (now English) information AND from the original source text — checking date columns, time columns, day-of-week labels, and any notes/remarks/footnotes. Convert every date to ISO 8601 and every time to 24h HH:MM as specified below.
4. Only after titles are translated and dates/times are extracted, form the final event objects. Every event MUST have either a concrete date (start) OR a day_of_week plus start_time — do not leave both empty if any date/time clue exists anywhere in the document for that event.
...
- Chinese CALENDAR dates (a month + day, e.g. "九月八日" / "9月8日" = September 8) MUST be converted to a concrete ISO 8601 date and placed in the start field (and end field), NOT merely turned into a day_of_week. Use the year that makes the date fall within the document's term; if unknown, use the current year. Example: "九月八日" with a 09:00 start → start "2026-09-08T09:00:00". Only use day_of_week (without a concrete date) when the source gives ONLY a weekday (周一/星期三) and no calendar date.
- Parse natural-language Chinese weekday names for the day_of_week field when no calendar date is available: "周一" / "星期一" / "礼拜一" → "Monday"; "周三" → "Wednesday". Map the full set: 周一/星期一=Monday, 周二/星期二=Tuesday, 周三/星期三=Wednesday, 周四/星期四=Thursday, 周五/星期五=Friday, 周六/星期六=Saturday, 周日/星期日/周天=Sunday.
